...
import { Plus, Pencil, Archive, FileText, Settings } from "lucide-react";
...
export default function AdminTemplateListPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    onError: () => toast.error("Failed to load templates."),
  } as any);
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Create and manage document templates</p>
            </div>
          </div>
          <button
            id="admin-new-template-btn"
                ...
                {isLoading
                  ...
                  : templates.map((tpl: any) => (
                      <tr key={tpl.id} className="transition hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-gray-500">{tpl.element_count ?? "—"}</td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex gap-2">
                            ...
                            <button
                              id={`admin-archive-tpl-${tpl.id}`}
                              onClick={() => setArchiveId(tpl.id)}
                              disabled={tpl.status === "archived"}
                              className="flex items-center gap-1 rounded-lg border border-amber-200 px-3 py-1.5 text-xs font-medium text-amber-600 hover:bg-amber-50 transition-colors disabled:opacity-40"
                            >
